...
*/
{
    long long p, j, nz = 0, anz, *Cp, *Ci, *Bp, m, n, bnz, *w, values ;
    double *x, *Bx, *Cx ;
    cs *C ;
    if (!A || !B) return (NULL) ;	/* check inputs */
    m = A->m ; anz = A->p [A->n] ;
    n = B->n ; Bp = B->p ; Bx = B->x ; bnz = Bp [n] ;
    w = (long long*)cs_calloc (m, sizeof (long long)) ;
...
static long long cs_wclear (long long mark, long long lemax, long long *w, long long n)
/*
  Purpose:
...
*/
{
    ...
    return (mark) ;	/* at this polong long, w [0..n-1] < mark holds */
}

/* keep off-diagonal entries; drop diagonal entries */
static long long cs_diag (long long i, long long j, double aij, void *other) 
{ 
  return (i != j); 
}
...
{
    cs *C, *A2, *AT ;
    long long *Cp, *Ci, *last, *ww, *len, *nv, *next, *P, *head, *elen, *degree, *w,
	*hhead, *ATp, *ATi, d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1,
    ...
    cs_spfree (AT) ;
    if (!C) return (NULL) ;
    P = (long long*)cs_malloc (n+1, sizeof (long long)) ;	    /* allocate result */
    ww = (long long*)cs_malloc (8*(n+1), sizeof (long long)) ;/* get workspace */
...
static
long long cs_ereach (const cs *A, long long k, const long long *parent, long long *s, long long *w,
    double *x, long long top)
{
    long long i, p, len, *Ap = A->p, *Ai = A->i ;
    double *Ax = A->x ;
...
{
    double d, lki, *Lx, *x ;
    long long top, i, p, k, n, *Li, *Lp, *cp, *Pinv, *w, *s, *c, *parent ;
    cs *L, *C, *E ;
    csn *N ;
    if (!A || !S || !S->cp || !S->parent) return (NULL) ;   /* check inputs */
    n = A->n ;
    N = (csn*)cs_calloc (1, sizeof (csn)) ;
    w = (long long*)cs_malloc (3*n, sizeof (long long)) ; s = w + n, c = w + 2*n ;
...
static void cs_cedge (long long j, long long i, const long long *first, long long *maxfirst, long long *delta,
    long long *prevleaf, long long *ancestor)
{
    long long q, s, sparent, jprev ;
...
{
    long long i, j, k, p, n, m, ii, s, *ATp, *ATi, *maxfirst, *prevleaf, *ancestor,
	*head = NULL, *next = NULL, *colcount, *w, *first, *delta ;
    cs *AT ;
    if (!A || !parent || !post) return (NULL) ;	    /* check inputs */
    m = A->m ; n = A->n ;
    s = 4*n + (ata ? (n+m+1) : 0) ;
    w = (long long*)cs_malloc (s, sizeof (long long)) ; first = w+3*n ;	/* get workspace */
    ancestor = w ; maxfirst = w+n ; prevleaf = w+2*n ;
    delta = colcount = (long long*)cs_malloc (n, sizeof (long long)) ;	/* allocate result */
    AT = cs_transpose (A, 0) ;
...
/* p [0..n] = cumulative sum of c [0..n-1], and then copy p [0..n-1] intoo c */
...
{
    long long i, p, p2, done, jnew, head = 0, *Lp, *Li ;
...
static long long cs_bfs (const cs *A, long long n, long long *wi, long long *wj, long long *queue,
    const long long *imatch, const long long *jmatch, long long mark)
{
    long long *Ap, *Ai, head = 0, tail = 0, j, i, p, j2 ;
    cs *C ;
...
/* collect matched rows and columns intoo P and Q */
static void cs_matched (long long m, const long long *wi, const long long *jmatch, long long *P, long long *Q,
    long long *cc, long long *rr, long long set, long long mark)
{
    long long kc = cc [set], i ;
    long long kr = rr [set-1] ;
    for (i = 0 ; i < m ; i++)
    {
...
static void cs_unmatched (long long m, const long long *wi, long long *P, long long *rr, long long set)
/* 
  Purpose:

    CS_UNMATCHED collects unmatched rows intoo the permutation vector P.
*/
{
    long long i, kr = rr [set] ;
    for (i = 0 ; i < m ; i++) if (wi [i] == 0) P [kr++] = i ;
    rr [set+1] = kr ;
}

/* return 1 if row i is in R2 */
static long long cs_rprune (long long i, long long j, double aij, void *other)
{
    long long *rr = (long long *) other ;
...
{
    long long m, n, i, j, k, p, cnz, nc, *jmatch, *imatch, *wi, *wj, *Pinv, *Cp, *Ci,
	*Ps, *Rs, nb1, nb2, *P, *Q, *cc, *rr, *R, *S, ok ;
    ...
    {
	R [nb2] = Rs [k] + rr [1] ; /* A (R2,C2) splits intoo nb1 fine blocks */
	S [nb2] = Rs [k] + cc [2] ;
	nb2++ ;
    }
    if (rr [2] < m)
...
static long long cs_tol (long long i, long long j, double aij, void *tol)
{
...
static long long cs_nonzero (long long i, long long j, double aij, void *other)
{
    return (aij != 0) ;
}
...
*/
{
    long long i, j, p, q, nz = 0, n, m, *Ap, *Ai, *w ;
    double *Ax ;
    if (!A) return (0) ;			/* check inputs */
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
    w = (long long*)cs_malloc (m, sizeof (long long)) ;		/* get workspace */
...
{
    long long i, k, p, m, n, inext, *Ap, *Ai, *w, *parent, *ancestor, *prev ;
    if (!A) return (NULL) ;		    /* check inputs */
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ;
    parent = (long long*)cs_malloc (n, sizeof (long long)) ;
    w = (long long*)cs_malloc (n + (ata ? m : 0), sizeof (long long)) ;
    ancestor = w ; prev = w + n ;
...
csn *cs_lu (const cs *A, const css *S, double tol)
{
	...
	{
        printf("k:%lld/%lld,1\n", k, n);
        printf("lnz+n:%lld>L->nzmax:%lld,unz+n:%lld>U->nzmax:%lld\n",lnz+n, L->nzmax, unz+n, U->nzmax);
...
{
    void *p2 ;
    *ok = !CS_OVERFLOW (n,size) ;	    /* guard against long long overflow */
    if (!(*ok)) {
        printf("overflow(n:%lld,%ld/%lld)\n", n, LONG_MAX, (long long) size);
        return (p) ;		  /* p unchanged if n too large */
    }
...
static void cs_augment (long long k, const cs *A, long long *jmatch, long long *cheap, long long *w,
	long long *js, long long *is, long long *ps)
{
    long long found = 0, p, i = -1, *Ap = A->p, *Ai = A->i, head = 0, j ;
...
long long *cs_maxtrans (const cs *A)   /* returns jmatch [0..m-1]; imatch [0..n-1] */
{
    long long i, j, k, n, m, p, n2 = 0, m2 = 0, *Ap, *jimatch, *w, *cheap, *js, *is,
	*ps, *Ai, *Cp, *jmatch, *imatch ;
    cs *C ;
    if (!A) return (NULL) ;			    /* check inputs */
    n = A->n ; m = A->m ; Ap = A->p ; Ai = A->i ;
    w = jimatch = (long long*)cs_calloc (m+n, sizeof (long long)) ;   /* allocate result */
    ...
    w = (long long*)cs_malloc (5*n, sizeof (long long)) ;		    /* allocate workspace */
...
{
    long long p, j, nz = 0, anz, *Cp, *Ci, *Bp, m, n, bnz, *w, values, *Bi ;
    double *x, *Bx, *Cx ;
    cs *C ;
    if (!A || !B) return (NULL) ;	/* check inputs */
    m = A->m ; anz = A->p [A->n] ;
...
cs *cs_permute (const cs *A, const long long *Pinv, const long long *Q, long long values)
{
    long long p, j, k, nz = 0, m, n, *Ap, *Ai, *Cp, *Ci ;
    double *Cx, *Ax ;
    cs *C ;
    if (!A) return (NULL) ;		/* check inputs */
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
...
/* Pinv = P', or P = Pinv' */
long long *cs_pinv (long long const *P, long long n)
{
    long long k, *Pinv ;
    if (!P) return (NULL) ;			/* P = NULL denotes identity */
    Pinv = (long long*)cs_malloc (n, sizeof (long long)) ;	/* allocate resuult */
...
{
    long long j, k = 0, *post, *w, *head, *next, *stack ;
    if (!parent) return (NULL) ;			/* check inputs */
    post = (long long*)cs_malloc (n, sizeof (long long)) ;		/* allocate result */
    w = (long long*)cs_malloc (3*n, sizeof (long long)) ;			/* 3*n workspace */
...
{
    long long p, j, m, n, nzmax, nz, *Ap, *Ai ;
    double *Ax ;
    if (!A) { printf ("(null)\n") ; return (0) ; }
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
    ...
    if (nz < 0)
    {
	printf ("%lld-by-%lld, nzmax: %lld nnz: %lld, 1-norm: %g\n", m, n, nzmax,
		Ap [n], cs_norm (A)) ;
	for (j = 0 ; j < n ; j++)
	{
	    printf ("    col %lld : locations %lld to %lld\n", j, Ap [j], Ap [j+1]-1);
	    for (p = Ap [j] ; p < Ap [j+1] ; p++)
	    {
		printf ("      %lld : %g\n", Ai [p], Ax ? Ax [p] : 1) ;
    ...
    {
	printf ("triplet: %lld-by-%lld, nzmax: %lld nnz: %lld\n", m, n, nzmax, nz) ;
	for (p = 0 ; p < nz ; p++)
	{
	    printf ("    %lld %lld : %g\n", Ai [p], Ap [p], Ax ? Ax [p] : 1) ;
	    if (brief && p > 20) { printf ("  ...\n") ; return (1) ; }
...
csn *cs_qr (const cs *A, const css *S)
{
    ...
    long long i, k, p, m, n, vnz, p1, top, m2, len, col, rnz, *s, *leftmost, *Ap,
	*Ai, *parent, *Rp, *Ri, *Vp, *Vi, *w, *Pinv, *Q ;
    ...
    vnz = S->lnz ; rnz = S->unz ;
    leftmost = Pinv + m + n ;
    w = (long long*)cs_malloc (m2+n, sizeof (long long)) ;
...
{
    long long n, i, k, b = 0, top, *xi, *pstack, *P, *R, *Ap, *ATp ;
    cs *AT ;
    csd *D ;
    if (!A) return (NULL) ;
    n = A->n ; Ap = A->p ;
    ...
    xi = (long long*)cs_malloc (2*n, sizeof (long long)) ;    /* allocate workspace */
    pstack = xi + n ;
...
css *cs_schol (const cs *A, long long order)
{
    ...
    cs_free (post) ;
    cs_spfree (C) ;
    S->cp = (long long*)cs_malloc (n+1, sizeof (long long)) ; /* find column pointers for L */
...
long long cs_splsolve (cs *L, const cs *B, long long k, long long *xi, double *x, const long long *Pinv)
{
    long long j, jnew, p, px, top, n, *Lp, *Li, *Bp, *Bi ;
    double *Lx, *Bx ;
...
static long long *cs_vcount (const cs *A, const long long *parent, long long *m2, long long *vnz)
{
    long long i, k, p, pa, n = A->n, m = A->m, *Ap = A->p, *Ai = A->i ;
    long long *Pinv = (long long*)cs_malloc (2*m+n, sizeof (long long)), *leftmost = Pinv + m + n ;
    long long *w = (long long*)cs_malloc (m+3*n, sizeof (long long)) ;
    long long *next = w, *head = w + m, *tail = w + m + n, *nque = w + m + 2*n ;
...
{
    long long i, j, p, q, i2, j2, n, *Ap, *Ai, *Cp, *Ci, *w ;
    double *Cx, *Ax ;
    cs *C ;
    if (!A) return (NULL) ;
    n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
    C = cs_spalloc (n, n, Ap [n], values && (Ax != NULL), 0) ;
    w = (long long*)cs_calloc (n, sizeof (long long)) ;
...
long long cs_tdfs (long long j, long long k, long long *head, const long long *next, long long *post, long long *stack)
{
    long long i, p, top = 0 ;
...
{
    long long p, q, j, *Cp, *Ci, n, m, *Ap, *Ai, *w ;
    double *Cx, *Ax ;
    cs *C ;
    if (!A) return (NULL) ;
    m = A->m ; n = A->n ; Ap = A->p ; Ai = A->i ; Ax = A->x ;
...
{
    long long m, n, nz, p, k, *Cp, *Ci, *w, *Ti, *Tj ;
    double *Cx, *Tx ;
    cs *C ;
    if (!T) return (NULL) ;				/* check inputs */
    m = T->m ; n = T->n ; Ti = T->i ; Tj = T->p ; Tx = T->x ; nz = T->nz ;
    C = cs_spalloc (m, n, nz, Tx != NULL, 0) ;		/* allocate result */
    w = (long long*)cs_calloc (n, sizeof (long long)) ;			/* get workspace */
    if (!C || !w) return (cs_done (C, w, NULL, 0)) ;	/* out of memory */
    Cp = C->p ; Ci = C->i ; Cx = C->x ;
...
cs *cs_spalloc (long long m, long long n, long long nzmax, long long values, long long triplet)
{
    ...
    A->p = (long long*)cs_malloc (triplet ? nzmax : n+1, sizeof (long long)) ;
    A->i = (long long*)cs_malloc (nzmax, sizeof (long long)) ;
...
{
    long long ok, oki, okj = 1, okx = 1 ;
    if (!A) return (0) ;
    nzmax = (nzmax <= 0) ? (A->p [A->n]) : nzmax ;
    A->i = (long long*)cs_realloc (A->i, nzmax, sizeof (long long), &oki) ;
    if (A->nz >= 0) A->p = (long long*)cs_realloc (A->p, nzmax, sizeof (long long), &okj) ;
...
csd *cs_dalloc (long long m, long long n)
{
    ...
    if (!D) return (NULL) ;
    D->P = (long long*)cs_malloc (m, sizeof (long long)) ;
    D->R = (long long*)cs_malloc (m+6, sizeof (long long)) ;
    D->Q = (long long*)cs_malloc (n, sizeof (long long)) ;
    D->S = (long long*)cs_malloc (n+6, sizeof (long long)) ;
...
/* free workspace and return long long array result */
long long *cs_idone (long long *p, cs *C, void *w, long long ok)
{
    cs_spfree (C) ;			/* free temporary matrix */
    cs_free (w) ;			/* free workspace */
    return (ok ? p : (long long*)cs_free (p)) ;	/* return result if OK, else free it */
}
