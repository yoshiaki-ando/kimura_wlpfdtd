#define HIGHER_FREQUENCY
//#define LOWER_FREQUENCY

#ifndef __WLP_FDTD2D_H__
#define __WLP_FDTD2D_H__
...
#ifdef HIGHER_FREQUENCY // 0.1-10Hz
...
/***********jammu&kashmir***********/
constexpr double RX {1500.0e3};
constexpr double RY {100.0e3};
constexpr double DX {RX/300};
...
constexpr double Distance_btw_Src_Obs { 962.0e3 };
constexpr double X_SOURCE {(RX/2.0) - Distance_btw_Src_Obs/2.0};
constexpr double Y_SOURCE {Y_SURFACE - 25.0e3}; //ISC
...
/***************追加**********************/
#ifdef HIGHER_FREQUENCY // 0.1-10Hz
constexpr double Rx_total { 5.e7 }; // 全解析領域の全長

// 増加率から不均一セル数を求める
constexpr double dx_max { 1.e6 };  // 最大セルサイズ(x方向)
constexpr double r_target { 1.5 }; // 不均一セルの増加率
